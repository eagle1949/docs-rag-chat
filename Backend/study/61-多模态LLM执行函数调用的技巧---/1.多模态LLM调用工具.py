...
class GaodeWeatherArgsSchema(BaseModel):
    city: str = Field(description="需要查询天气预报的目标城市，例如：深圳")
...
# 2.构建LLM并绑定工具
llm = ChatOpenAI(model="moonshot-v1-8k-vision-preview")
...
pic_url = "https://imooc-langchain.shortvar.com/guangzhou.jpg"
response = requests.get(pic_url)
# 我们使用标准库 base64.b64encode 函数将图片编码成 base64 格式的 image_url
image_data = response.content
image_url = f"data:image/{os.path.splitext(pic_url)[1]};base64,{base64.b64encode(image_data).decode('utf-8')}"
 
print(chain.invoke(image_url))